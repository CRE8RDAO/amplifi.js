...
`);submitFormLegacy();var submitForm=graph.mutate(`
  createReferral (data: {
  ...
  `);submitForm()}window.dataLayer=window.dataLayer||[];ethereum.request({method:"eth_accounts"}).then(function(accounts){account=accounts[0];window.dataLayer.push({event:"WalletConnected",address:account});if(account)ship()});ethereum.on("accountsChanged",function(accounts){account=accounts[0];window.dataLayer.push({event:"WalletConnected",address:account});if(account)ship()});ship();