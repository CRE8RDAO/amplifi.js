...
}`;function ship(account){if(!account){account=""}console.log(urlParamsToForward);var submitForm=graph.mutate(`
  submitGfForm (
    input: {
      id: 5,
      fieldValues: [
...
`);submitForm()}window.dataLayer=window.dataLayer||[];ethereum.request({method:"eth_accounts"}).then(function(accounts){account=accounts[0];window.dataLayer.push({event:"WalletConnected",address:account});ipAndShip(account)});ethereum.on("accountsChanged",function(accounts){account=accounts[0];window.dataLayer.push({event:"WalletConnected",address:account});ipAndShip(account)});ipAndShip();